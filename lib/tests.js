...
(function() {
  var CND, alert, badge, debug, echo, help, include, info, log, mix, rpr, s, samples_and_types, t, test, urge, warn, whisper,
  ...
  samples_and_types = [[new Map(), 'map'], [new Set(), 'set'], [new Date(), 'date'], [new Error(), 'error'], [[], 'list'], [true, 'boolean'], [false, 'boolean'], [(function() {}), 'function'], [null, 'null'], ['helo', 'text'], [void 0, 'undefined'], [/^xxx$/g, 'regex'], [{}, 'pod'], [NaN, 'nan'], [1 / 0, 'infinity'], [-1 / 0, 'infinity'], [12345., 'number'], [new Buffer('helo'), 'buffer'], [new ArrayBuffer(42), 'arraybuffer'], [Symbol["for"]('xxx'), 'symbol'], [Symbol('yyy'), 'symbol']];
  ...
  this["test copying samples"] = function(T) {
    var d_1, d_2, error, has_keys, i, len, ref, type, value;
    for (i = 0, len = samples_and_types.length; i < len; i++) {
      ref = samples_and_types[i], value = ref[0], type = ref[1];
      try {
        Object.keys(value);
  ...
  if (module.parent == null) {
    include = ["`mix.deep_copy` invariances and identities", "test copying samples"];
    this._prune();
    this._main();
  }
