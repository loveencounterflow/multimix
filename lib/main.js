...
(function() {
  ...
  MULTIMIX.mix = function(L, mixins, reducers, root, selector) {
    var R, S, attributes, copy, description, i, j, len, len1, mixin, mx_key, mx_value, partial_mixin, partial_mixins, rd_key, rd_value, reducer, ref, ref1, ref2, seed, seen, type;
    if (root == null) {
      root = null;
    }
    if (selector == null) {
    ...
    ref = mixins, seed = ref[0], mixins = 2 <= ref.length ? slice.call(ref, 1) : [];
    type = CND.type_of(seed);
    description = (ref1 = L.type_descriptions[type]) != null ? ref1 : L.type_descriptions[σ_unknown_type];
    ...
    for (j = 0, len1 = mixins.length; j < len1; j++) {
      ...
      for (mx_key in mixin) {
        ...
        S.root = root;
        S.current = R;
        S.reducer_name = (ref2 = S.reducers[mx_key]) != null ? ref2 : S.reducer_fallback;
