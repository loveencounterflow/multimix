...
(function() {
  ...
  MULTIMIX.mix = function(me, reducers, mixins, root, selector) {
    var R, S, i, j, len, len1, mixin, mx_key, mx_value, partial_mixin, partial_mixins, rd_key, rd_value, reducer, reducer_name, ref;
    if (root == null) {
      root = null;
    }
    if (selector == null) {
    ...
    for (j = 0, len1 = mixins.length; j < len1; j++) {
      ...
      for (mx_key in mixin) {
        ...
        S.root = root;
        S.current = R;
        if (me.REDUCERS[σ_reject](S, R, mx_key, mx_value)) {
          continue;
        }
        reducer_name = (ref = S.reducers[mx_key]) != null ? ref : S.reducer_fallback;
